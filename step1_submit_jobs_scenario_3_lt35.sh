...
do
    ...
    # Adjust the CHiPs schedule for those in younger age groups.  
    ...
    do
        ...
        do
            ...
            python python/scale_chips_coverage_by_age.py --uptake_file "$input_dir/$input_file" \
                --start_age 18 --end_age 34 --genders "M" "F" --other_coverage "0.0"
        done
    done
    
    # Submit the job (as an array job) for the community in question
    qsub -N closing_gaps_CL${community}_${scenario} "./jobs/closing_gaps_${scenario}.sh" $community $input_dir $output_dir
